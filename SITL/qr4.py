...
"""
ANKA - ArduPlane SITL / Gazebo kontrollü ve düz GUIDED dalış sistemi
...
3) AUTO modunda WP1-WP5, sanal WP ve dalış WP takip edilir; WP5 sonrası seyir irtifası korunur.
4) Dalış WP aktifken AUTO'nun hedef doğrultusuna oturduğu ilk uygun anda GUIDED'e geçilir.
   GUIDED'da en az 6 s düz uçulur; son 2 s boyunca doğrultu ve roll kesintisiz doğrulanmadan dalış başlamaz.
5) Uçuş-yolu açısı, gecikme telafili PID ile yaklaşık 48 derece aşağıda tutulur.
6) Gaz dalışta %35'e kademeli iner. Dalış 50.0 m bağıl irtifaya kadar sürer;
   50.0 m görüldüğü anda RECOVERY başlar.
7) Dalış, toparlanma ve düz uçuşta kilitlenen GPS yer izi küçük roll komutlarıyla korunur.
8) ArduPlane SET_ATTITUDE_TARGET yaw alanını doğrudan dümen komutuna çevirdiği için quaternion yaw=0 gönderilir.
   Böylece V-kuyrukta tam sol/sağ dümen oluşmaz; yön kontrolü yalnızca küçük roll/bank komutlarıyla yapılır.
9) Toparlanma gazı yumuşak biçimde %100'e çıkar ve iki aşamalı pull-out uygulanır.
10) 100 m sonrasında araç aynı doğrultuda düz uçuşa devam eder; sonraki WP'ye dönmez.

SİMÜLASYON BAĞLANTISI: udp:127.0.0.1:14550
30 m otomatik RTL irtifa sınırı bu simülasyon sürümünde kaldırılmıştır.
...
- Dalış, GUIDED'a geçer geçmez başlamaz. En az 6 s HOLD yapılır ve son 2 s
  boyunca heading hatası ile gerçek roll uygun kalmadan dalışa izin verilmez.
...
# Dalış uçuş-yolu açısı PID'i. Roll/yön kontrolüne dokunulmadan yalnızca
# boylamsal kontrol ayarlanmıştır. Hedef 48°'dir. GPS uçuş-yolu açısının değişim
# hızı ileriye taşınarak uçağın yaklaşık 0.65 s sonraki PATH açısı tahmin edilir;
# frenleme bu tahmine göre erkenden başlatılır. Gerçek PATH 48°'yi aşarsa
# burun-aşağı komutu tamamen bırakılıp pozitif pitch ile kesin fren uygulanır.
DIVE_BASE_PITCH_DEG = -42.0
DIVE_PATH_KP = 0.55
DIVE_PATH_KI = 0.002
DIVE_PATH_KD = 0.15
...
DIVE_MAX_PITCH_DEG = 18.0
DIVE_PATH_LOOKAHEAD_SEC = 0.65
DIVE_PATH_BRAKE_START_DEG = 34.0
DIVE_PATH_BRAKE_LEVEL_DEG = 40.0
DIVE_PATH_BRAKE_HARD_DEG = 46.0
DIVE_PATH_BRAKE_START_PITCH_DEG = -24.0
DIVE_PATH_BRAKE_LEVEL_PITCH_DEG = 4.0
DIVE_PATH_BRAKE_HARD_PITCH_DEG = 16.0

# GPS PATH türevi 10 Hz telemetri nedeniyle kısa süreli sıfıra düşebilir. Yalnızca
# öngörülen açı kullanılırsa fren bir çevrim bırakılıp yeniden burun-aşağı komut
# üretilebilir. Bu gerçek PATH tabanı, 36° sonrasında freni kesintisiz tutar.
DIVE_PATH_ACTUAL_BRAKE_START_DEG = 36.0
DIVE_PATH_ACTUAL_BRAKE_LEVEL_DEG = 40.0
DIVE_PATH_ACTUAL_BRAKE_HARD_DEG = 44.0
DIVE_PATH_ACTUAL_BRAKE_START_PITCH_DEG = -20.0
DIVE_PATH_ACTUAL_BRAKE_LEVEL_PITCH_DEG = 2.0
DIVE_PATH_ACTUAL_BRAKE_HARD_PITCH_DEG = 14.0
DIVE_PATH_OVERSHOOT_PITCH_DEG = 18.0
...
DIVE_ENTRY_TRANSITION_SEC = 1.20
...
DIVE_PITCH_NOSE_DOWN_RATE_DEG_PER_SEC = 40.0
DIVE_PITCH_NOSE_UP_RATE_DEG_PER_SEC = 80.0
...
try:
    while True:
        ...
        if phase == "DIVE" and loop_now - last_control_send >= CONTROL_PERIOD:
            ...
            if path_angle is None:
                # GPS hız vektörü geçici olarak yoksa güvenli temel pitch kullan.
                ...
                # Gerçek dalış açısı 48°'den küçükse hata pozitiftir;
                # pitch daha negatif yapılarak dalış dikleştirilir.
                pitch_command = DIVE_BASE_PITCH_DEG - pid_output

                # error türevi, PATH değişim hızının ters işaretlisidir.
                # İleriye bakış sayesinde hızlı dikleşme gerçek PATH 48° olmadan
                # fark edilir ve uçak atalet kazanmadan frenlemeye başlanır.
                path_rate_down_deg_s = max(-dive_path_derivative_filtered, 0.0)
                predicted_path_angle = (
                    path_angle
                    + path_rate_down_deg_s * DIVE_PATH_LOOKAHEAD_SEC
                )

                if predicted_path_angle >= DIVE_PATH_BRAKE_START_DEG:
                    if predicted_path_angle < DIVE_PATH_BRAKE_LEVEL_DEG:
                        brake_ratio = smoothstep01(
                            (predicted_path_angle - DIVE_PATH_BRAKE_START_DEG)
                            / (DIVE_PATH_BRAKE_LEVEL_DEG - DIVE_PATH_BRAKE_START_DEG)
                        )
                        brake_pitch_floor = (
                            DIVE_PATH_BRAKE_START_PITCH_DEG * (1.0 - brake_ratio)
                            + DIVE_PATH_BRAKE_LEVEL_PITCH_DEG * brake_ratio
                        )
                    else:
                        brake_ratio = smoothstep01(
                            (predicted_path_angle - DIVE_PATH_BRAKE_LEVEL_DEG)
                            / (DIVE_PATH_BRAKE_HARD_DEG - DIVE_PATH_BRAKE_LEVEL_DEG)
                        )
                        brake_pitch_floor = (
                            DIVE_PATH_BRAKE_LEVEL_PITCH_DEG * (1.0 - brake_ratio)
                            + DIVE_PATH_BRAKE_HARD_PITCH_DEG * brake_ratio
                        )
                    pitch_command = max(pitch_command, brake_pitch_floor)

                # Öngörü filtresi anlık olarak gevşese bile gerçek PATH 36°'yi
                # geçtiyse fren komutunu bırakma. Bu taban, önceki testte görülen
                # +3.2° -> -23.3° komut salınımını ve ikinci dikleşmeyi önler.
                if path_angle >= DIVE_PATH_ACTUAL_BRAKE_START_DEG:
                    if path_angle < DIVE_PATH_ACTUAL_BRAKE_LEVEL_DEG:
                        actual_brake_ratio = smoothstep01(
                            (path_angle - DIVE_PATH_ACTUAL_BRAKE_START_DEG)
                            / (
                                DIVE_PATH_ACTUAL_BRAKE_LEVEL_DEG
                                - DIVE_PATH_ACTUAL_BRAKE_START_DEG
                            )
                        )
                        actual_brake_pitch_floor = (
                            DIVE_PATH_ACTUAL_BRAKE_START_PITCH_DEG
                            * (1.0 - actual_brake_ratio)
                            + DIVE_PATH_ACTUAL_BRAKE_LEVEL_PITCH_DEG
                            * actual_brake_ratio
                        )
                    else:
                        actual_brake_ratio = smoothstep01(
                            (path_angle - DIVE_PATH_ACTUAL_BRAKE_LEVEL_DEG)
                            / (
                                DIVE_PATH_ACTUAL_BRAKE_HARD_DEG
                                - DIVE_PATH_ACTUAL_BRAKE_LEVEL_DEG
                            )
                        )
                        actual_brake_pitch_floor = (
                            DIVE_PATH_ACTUAL_BRAKE_LEVEL_PITCH_DEG
                            * (1.0 - actual_brake_ratio)
                            + DIVE_PATH_ACTUAL_BRAKE_HARD_PITCH_DEG
                            * actual_brake_ratio
                        )
                    pitch_command = max(pitch_command, actual_brake_pitch_floor)

                # 48° gerçek üst sınırı: aşıldığında hiçbir negatif pitch
                # komutuna izin verme; bir sonraki döngüde PATH'i geri bastır.
                if path_angle >= TARGET_FLIGHT_PATH_DOWN_DEG:
                    pitch_command = max(
                        pitch_command,
                        DIVE_PATH_OVERSHOOT_PITCH_DEG,
                    )
